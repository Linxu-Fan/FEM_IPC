...
namespace extractCrackSurface 
{
	...
	weightAndDreri calWeight(double dx, Eigen::Vector3d pos)
	{
		...
		weightAndDreri res(ppIndex, space, weight, deltaWeight);
    ...
    std::tuple<bool, objMeshFormat, objMeshFormat, std::vector<objMeshFormat>> extractCrackSurf(std::vector<Particle>* particlesRaw, CRACKParamters param)
    {
        ...
        if (interiorFragmentIndex.size() == 0) // all fargments are not interior
        {
            ...
            for (it = interiorFragmentIndex.begin(); it != interiorFragmentIndex.end(); ++it) {
                ...
                if (surroundingVolume < 0) {
                    bool findCrackSurface = false;
                    objMeshFormat nocrackmesh;
                    std::vector<objMeshFormat> noFragments;
                    std::tuple<bool, objMeshFormat, objMeshFormat, std::vector<objMeshFormat>> resultReturn(findCrackSurface, nocrackmesh, nocrackmesh, noFragments);
                    return resultReturn;
                }
            }
        ...
        for (int i = 0; i < allFragmentsRemoveInterior.size(); i++) {
            ...
            objMeshFormat fragmentObj;
            fragmentObj.facesPolygonal = facesEachFrag;
        ...
        else
        {
            ...
            crackSurfacePartialCut.facesPolygonal = facesTmp;
        }
        ...
        if (newAlgoRemoveDup)
        {
            ...
            crackSurfaceFullCut.facesPolygonal = reIndexedFaces;
        ...
        else
        {
            ...
            crackSurfaceFullCut.facesPolygonal = faces;
        }
