		...
		for (int g = 0; g < nodesVec.size(); g++)
		{
			for (int p = 0; p < nodesVec[g].supportParticles.size(); p++)
			{
				...
				double weight = nodesVec[g].supportParticlesWeight[p];
				{
					...
					// nodesVec[g].force += -weight / (param.dx * param.dx / 4.0) * param.dt * particles[parPosInParticleVec].volume * (particles[parPosInParticleVec].F).determinant() * (particles[parPosInParticleVec].cauchyStress * (nodesVec[g].position - particles[parPosInParticleVec].position)).transpose();
				}
			}
		}
...
#pragma omp parallel for num_threads(param.numOfThreads)
	...
	void advanceStep(std::vector<MPMParticle>& particles, MPMParamters& param, std::vector<Vector6d>& contactForce) // prticle vector, timestep
	{
	...
	std::tuple<bool, objMeshFormat, objMeshFormat, std::vector<objMeshFormat>> crackSimulation(std::vector<MPMParticle>& particles, MPMParamters& param, std::vector<Vector6d>& contactForce, int num_timestep)
	{
		...
		{
			advanceStep(particles, param, contactForce);
