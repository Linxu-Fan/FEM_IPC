...
//		{
//			if (timestep >= tetSimMesh.boundaryCondition_node[i].appliedTime[0] && timestep <= tetSimMesh.boundaryCondition_node[i].appliedTime[1])
//			{
...
//		}
//	}
//	std::vector<double> AB_ine_energy_vec(tetSimMesh.num_meshes);
...
//	std::vector<double> AB_aff_energy_vec(tetSimMesh.num_meshes);
//#pragma omp parallel for num_threads(parameters.numOfThreads)
