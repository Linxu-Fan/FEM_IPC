...
//void implicitFEM_ABD_triMesh(Mesh_ABD& triSimMesh, FEMParamters& parameters)
//{
//
...
//		}
//
//		triSimMesh.pos_node_prev = triSimMesh.pos_node;
//		triSimMesh.translation_prev_ABD = triSimMesh.translation_ABD;
//		triSimMesh.deformation_prev_ABD = triSimMesh.deformation_ABD;
//
//		std::vector<Eigen::Vector3d> position_direction(triSimMesh.pos_node.size(), Eigen::Vector3d::Zero());
//
//		double lastEnergyVal = compute_IP_energy_ABD(triSimMesh, parameters, timestep);
//		for (int ite = 0; ite < 15; ite++)
//		{
//			std::vector<Eigen::Vector3d> currentPosition = triSimMesh.pos_node;
...
//			std::vector<Vector12d> direction_ABD = solve_linear_system_ABD(triSimMesh, parameters, timestep);
//			convert_to_position_direction(parameters, triSimMesh, direction_ABD, position_direction);
...
//
//
//			//std::cout << std::scientific << std::setprecision(4) << "triSimMesh.massMatrix_ABD[0] = " << std::endl << triSimMesh.massMatrix_ABD[0] << std::endl;
//
...
//			double step = calMaxStepSize(triSimMesh, parameters, timestep, position_direction);
//			//step = 1.0;
//			std::cout << "			Step forward = " << step << std::endl;
//			step_forward_ABD(parameters, triSimMesh, current_ABD_translation, current_ABD_deformation, direction_ABD, currentPosition, step);
//			double newEnergyVal = compute_IP_energy_ABD(triSimMesh, parameters, timestep);
...
//
//				step_forward_ABD(parameters, triSimMesh, current_ABD_translation, current_ABD_deformation, direction_ABD, currentPosition, step);
//				newEnergyVal = compute_IP_energy_ABD(triSimMesh, parameters, timestep);
//				std::cout << "				step = " << step << "; newEnergyVal = "
//					<< newEnergyVal << "; dis = " << triSimMesh.pos_node[7][2] - triSimMesh.pos_node[0][2]
//					<< std::endl;
...
//		for (int i = 0; i < triSimMesh.pos_node.size(); i++)
//		{
//			triSimMesh.vel_node[i] = (triSimMesh.pos_node[i] - triSimMesh.pos_node_prev[i]) / parameters.dt;
//
//			if (i == 0)
//			{
//				std::cout << "			triSimMesh.vel_node[0] = " << triSimMesh.vel_node[0].transpose() << std::endl;
//			}
//
//		}
//
...
//
//			int AB_index = triSimMesh.index_node_interior[i];
...
//
//			int AB_index = triSimMesh.index_node_surface[i];
...
//	energyVal += compute_Barrier_energy(triSimMesh, parameters, timestep);
//
//
//	return energyVal;
//}
...
//std::vector<Vector12d> solve_linear_system_ABD_triMesh(Mesh_ABD& triSimMesh, FEMParamters& parameters, int timestep)
//{
...
//	calContactInfo(triSimMesh, parameters, timestep, PG_PG, PT_PP, PT_PE, PT_PT, EE_EE);
//
//
...
//	for (int i = 0; i < triSimMesh.pos_node.size(); i++)
//	{
//		if (triSimMesh.boundaryCondition_node[i].type == 2)
//		{
//			Eigen::Matrix<double, 3, 12> Jx = build_Jx_matrix_for_ABD(triSimMesh.pos_node_Rest[i].transpose());
//
//			int AB_index = triSimMesh.index_node[i][0];
//			Eigen::Vector3d ext_force = compute_external_force(triSimMesh, i, timestep);
//			Vector12d affine_force_node = Jx.transpose() * ext_force;
//
//			affine_force[AB_index] += affine_force_node;
//		}
...
//		{
//			int ptInd = PG_PG[i][1];
//			Eigen::Vector3d P = triSimMesh.pos_node[ptInd];
...
//				actualStartIndex_grad, triSimMesh, ptInd,
//				z2, triSimMesh.boundaryVertices_area[ptInd], parameters, true);
//		}
//
...
//			int ptInd = cont_PT[1];
//			Eigen::Vector3d P = triSimMesh.pos_node[ptInd];
//			Eigen::Vector3i tri = triSimMesh.boundaryTriangles[cont_PT[2]];
//			Eigen::Vector3d A = triSimMesh.pos_node[tri[0]];
//			Eigen::Vector3d B = triSimMesh.pos_node[tri[1]];
//			Eigen::Vector3d C = triSimMesh.pos_node[tri[2]];
//			int type = cont_PT[3];
//
//			double dis2 = 0;
...
//				dis2, triSimMesh, parameters, true);
//
//		}
//
...
//			int e1p1 = triSimMesh.index_boundaryEdge[E1][0], e1p2 = triSimMesh.index_boundaryEdge[E1][1],
//				e2p1 = triSimMesh.index_boundaryEdge[E2][0], e2p2 = triSimMesh.index_boundaryEdge[E2][1];
//
//			Eigen::Vector3d P1 = triSimMesh.pos_node[e1p1];
//			Eigen::Vector3d P2 = triSimMesh.pos_node[e1p2];
//			Eigen::Vector3d Q1 = triSimMesh.pos_node[e2p1];
//			Eigen::Vector3d Q2 = triSimMesh.pos_node[e2p2];
//			int type = cont_EE[3];
//
//			double dis2 = 0;
...
//void step_forward_ABD_triMesh(FEMParamters& parameters, Mesh_ABD& triSimMesh, std::vector<Eigen::Vector3d>& current_ABD_translation,
...
//#pragma omp parallel for num_threads(parameters.numOfThreads)
//	for (int vI = 0; vI < triSimMesh.pos_node.size(); vI++)
//	{
//		Eigen::Matrix<double, 3, 12> Jx = build_Jx_matrix_for_ABD(triSimMesh.pos_node_Rest[vI].transpose());
//		int ABD_index = triSimMesh.index_node[vI][0];
//		triSimMesh.pos_node[vI] = currentPosition[vI] + step * Jx * ABD_direction[ABD_index];
//	}
//
//
//}
...
//void convert_to_position_direction_triMesh(FEMParamters& parameters, Mesh_ABD& triSimMesh, std::vector<Vector12d>& direction_ABD, std::vector<Eigen::Vector3d>& position_direction)
//{
//
//#pragma omp parallel for num_threads(parameters.numOfThreads)
//	for (int vI = 0; vI < triSimMesh.pos_node.size(); vI++)
...
//		//position_direction[vI] = current_position - triSimMesh.pos_node_prev[vI];
//
//
